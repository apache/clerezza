...
public final class Permissions
{
	...
	//caches disabled
    private static final Map m_permissionCache = new AbstractMap() {
    ...
    private static final class Entry extends WeakReference
    {
        ...
        public boolean equals(Object o)
        {
            ...
            if (entry == null)
            {
				System.out.println("Comparing cleared entry!");
                return false;
            }
    ...
    private void cleanUp(ReferenceQueue queue, Map cache)
    {
            ...
            (Entry) queue.poll())
        {
            synchronized (cache)
            {
				System.out.println("removing entry, current size "+cache.size());
                cache.remove(entry);
				System.out.println("new size "+cache.size());
            }
        }
    }
