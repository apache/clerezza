...
object Utils{
   ...
   */
  protected[interpreter] def nullOrElse[S, T](s: S)(f: S => T): T =
   ...
   */
  protected[interpreter] def valueOrElse[T](t: T)(default: => T) =
    if (t == null) default
    else t
   ...
   */
    protected[interpreter] def option[T](value: T): Option[T] =
