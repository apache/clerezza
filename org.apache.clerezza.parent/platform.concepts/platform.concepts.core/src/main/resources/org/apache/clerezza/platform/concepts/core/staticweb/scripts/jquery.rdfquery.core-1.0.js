...
(function ($) {
  ...
  $.rdf.fn = $.rdf.prototype = {
    ...
    /**
	 * Returns a {jQuery.rdf.graphNode} object the specified {jQuery.rdf.resource}. {jQuery.rdf.graphNode} provide methods for traversing through the graph.
	 * @returns {jQuery.rdf.graphNode}
	 */
	node: function (resource) {
		return $.rdf.graphNode.fn.init(this, resource);
	},
	
	/**
	 * Returns a {jQuery.rdf.rdfList} object for the specified {jQuery.rdf.resource}. This object can then be used for iterating through the list elements.
	 * @returns {jQuery.rdf.rdfList}
	 */
	list: function(resource) {
		return $.rdf.rdfList.fn.init(this.node(resource));
	},
  ...
  /**
   * <p>Creates a new {jQuery.rdf.graphNode} object. Rather than constructing a {jQuery.rdf.graphNode} using new, use the node()-method on a (jQuery.rdf} object. </p>
   * @class Represents a node in a rdf graph.
   * @param {jQuery.rdf} rdf The {jQuery.rdf) object that contains the graph
   * @param {jQuery.rdf.resource} resource The node in the graph
   * @see jQuery.rdf#node
   * @see jQuery.rdf.resource
   */
  $.rdf.graphNode = function (rdf, resource) {
	  return $.rdf.graphNode.fn.init(rdf, resource);
  }

  $.rdf.graphNode.fn = $.rdf.graphNode.prototype = {
    init: function (rdf, resource) {
	  this.rdf = rdf;
	  
	  /**
	   * This function is added to the {Object} that is also an {Array} returned by {jQuery.rdf.graphNode#get} and {jQuery.rdf.graphNode#getInverse}. The function tests if the {Array} contains a element with the same string representation as the specified {jQuery.rdf.resource}. 
	   */
	  this.contains = function(resource) 
		{
			var contained = false;
			 $.each(this, function() {
			      if(this.toString() == resource.toString()) {
			    	  contained = true;
			      }
			    });
			 return contained;
	  ...
	  /**
	   * This function is added to the {Object} that is also an {Array} returned by {jQuery.rdf.graphNode#get} and {jQuery.rdf.graphNode#getInverse}. The function applies the specified function to every {jQuery.rdf.graphNode in the {Array}.
	   */
	  this.each = function(func) {
		  $.each(this, func);
	  }
      return $.extend({}, resource, this);
    },

	/**
	 * Returns an {Object} that is a an {Array} and a {jQuery.rdf.graphNode}. The {Array} contains all {jQuery.rdf.graphNode}s, which are the object in triples which have this graph node as the subject and the specified property as property. Besides being an {Array} the returned object is also the first {GraphNode} of the {Array}.
	 * @param {jQuery.rdf.resource} property, The property of the graph node to travers.
	 * @returns {{Array}&jQuery.rdf.graphNode} the {jQuery.rdf.graphNode}/s that are object of triples, where this graph node is the subject and the specified property the property.
	 */
    get: function (property) {
		var matches = new Array();
		var i = 0;
		var rdf = this.rdf;
		rdf.where(($.browser.msie ? this.getString() : this) + " " + property + " ?object")
			.each(function () {
				matches[i++] = $.rdf.graphNode.fn.init(rdf, this.object);
		});
		if (i === 0) return null;
		var result = $.extend({}, matches[0], matches);
		result.length = matches.length;
		result.contains = this.contains;
		result.each = this.each;
		return result;
    },

	/**
	 * Returns an {Object} that is a an {Array} and a {jQuery.rdf.graphNode}. The {Array} contains all {jQuery.rdf.graphNode}s, which are the subject in triples which have this graph node as the object and the specified property as property. Besides being an {Array} the returned object is also the first {GraphNode} of the {Array}.
	 * @param {jQuery.rdf.resource} property, The property of the graph node to travers.
	 * @returns {{Array}&jQuery.rdf.graphNode} the {jQuery.rdf.graphNode}/s that are subject of triples, where this graph node is the object and the specified property the property.
	 */
	getInverse: function (property) {
		var matches = new Array();
		var i = 0;
		var rdf = this.rdf;
		rdf.where("?subject " + property + " " + this)
			.each(function () {
				matches[i++] = $.rdf.graphNode.fn.init(rdf, this.subject);
		});
		if (i === 0) return null;
		var result = $.extend({}, matches[0], matches);
		result.length = matches.length;
    ...
    /**
	 * Returns a {jQuery.rdf.rdfList} object for this graph node. This object can then be used for iterating through the list elements.
	 * @returns {jQuery.rdf.rdfList}
	 */
    list: function() {
    	return $.rdf.rdfList.fn.init(this);
    },
  ...
  /**
   * <p>Creates a new {jQuery.rdf.rdfList} object. Rather than constructing a {jQuery.rdf.rdfList} using new, use the list()-method on a (jQuery.rdf} or {jQuery.rdf.graphNode} object. </p>
   * @class An Iterator over a rdf list.
   * @param {jQuery.rdf.graphNode} listNode A node in the rdf list.
   * @see jQuery.rdf#node
   */
  $.rdf.rdfList = function (rdf, resource) {
	  return $.rdf.rdfList.fn.init(rdf, resource);
  }

  $.rdf.rdfList.fn = $.rdf.rdfList.prototype = {
    init: function (listNode) {
  	...
  	/**
     * Returns the next {jQuery.rdf.resource} from the list. Returns null if the list has no more elements.
     * @returns {null|jQuery.rdf.resource}
     */
  	next: function() {
		if (this.listNode.toString() == jQuery.rdf.nil) {							
			return null;
		}
		var result = this.listNode
			.get(jQuery.rdf.first);
	...
	/**
     * Call the specified function for each element in the list.
     */
	each: function(func) {
		var next, i = 0, elements = new Array();
